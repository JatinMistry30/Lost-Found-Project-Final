...
router.post('/:id/respond', getCurrentUserDataLog, async (req, res) => {
    ...
    try {
      ...
      const [reports] = await db.query(
        'SELECT finder_id, item_id FROM found_reports WHERE id = ?',
        [id]
      );
...
await db.query(
  ...
  [newStatus, report.item_id]
      ...
      const notificationMessage = status === 'accepted'
        ? `Your found item report has been accepted! The owner will contact you soon.`
        : `Your found item report has been rejected.`;
  
      await createNotification(
        report.finder_id,
        'Found Report Response',
        notificationMessage,
        id
      );
